...
app.controller('ctrl', function($scope) {
    ...
    var andNullORundefined = function (a, b) {return and(!isNullORundefined(a), !isNullORundefined(b))}
    
    $scope.quiz12result = 2.90;
    $scope.quiz13result = 0.01;
    $scope.quiz14result = 0.51;
    $scope.quiz15result = 0.34;
    $scope.quiz22result = 0.58;
    $scope.quiz23result = 0.08;
    $scope.quiz24result = 0.17;
    $scope.quiz25result = 0.04;
    $scope.quiz27result = 0.09;
    $scope.quiz28result = 0.30;
    ...
    $scope.$watch('[meanwage]', function(newValues,oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.quiz11result = $scope.quiz26result = null;
            return;
        }
    ...
    $scope.$watch('[hotwater]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.hotwaterSum = null;
            return;
        }
        
        var hotwater = newValues[0];
        var resultValue = Number((0.001270 * hotwater).toFixed(2));
        
        $scope.hotwaterSum = !isNullORundefined(resultValue) ? resultValue : null;
    });
    
    $scope.$watch('[water]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.waterSum = null;
            return;
        }
        
        var water = newValues[0];
        var resultValue = Number((0.001910 * water).toFixed(2));
        
        $scope.waterSum = !isNullORundefined(resultValue) ? resultValue : null;
    });
    
    $scope.$watch('[sewage]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.sewageSum = null;
            return;
        }
        
        var sewage = newValues[0];
        var resultValue = Number((0.003180 * sewage).toFixed(2));
        
        $scope.sewageSum = !isNullORundefined(resultValue) ? resultValue : null;
    });
    
    $scope.$watch('[electricity]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.electricitySum = null;
            return;
        }
        
        var electricity = newValues[0];
        var resultValue = Number((0.000640 * electricity).toFixed(2));
        
        $scope.electricitySum = !isNullORundefined(resultValue) ? resultValue : null;
    });
    
    $scope.$watch('[heat]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.heatSum = null;
            return;
        }
        
        var heat = newValues[0];
        var resultValue = Number((0.000020 * heat).toFixed(2));
        
        $scope.heatSum = !isNullORundefined(resultValue) ? resultValue : null;
    });
    
    $scope.$watch('[hotwaterSum, waterSum, sewageSum, electricitySum, heatSum]', function(newValues, oldValues) {
        if (!newValues.reduce(andNullORundefined, true)) {
            $scope.quiz21result = null;
            return;
        }
        
        var hotwaterSum = newValues[0];
        var waterSum = newValues[1];
        var sewageSum = newValues[2];
        var electricitySum = newValues[3];
        var heatSum = newValues[4];
        
        var resultValue = hotwaterSum + waterSum + sewageSum + electricitySum + heatSum;
        
        
        $scope.quiz21result = !isNullORundefined(resultValue) ? resultValue : null;
    });
    ...
    $scope.$watch('[quiz11result, quiz12result, quiz13result, quiz14result,' +
        ...
        $scope.result = !isNullORundefined(resultValue) ? resultValue : null;
    });
